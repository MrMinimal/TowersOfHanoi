...
/**
 * Provides a static method to pass the command line arguments passed to the
 * main() method.
 * <p>
 * The minimal disk amount is a default value of 4 disks.
 * 
...
public abstract class GameModeCreator {
	...
	private final static int DISK_INDEX = 0; // at which position in the
												// arguments the "disk amount"
												// is expected
	...
	/**
	 * Returns all the settings, input via the parameters of the main
	 * application
	 * 
	 * @param settings
	 *            All the parameters read via the command line. Interesting
	 *            arguments start at index 0.
	...
	public static Settings parseSettings(String[] settings) {
		...
		int delay = 1; // default value one second unless set otherwise via
						// arguments
		...
		for (int i = 0; i < settings.length; i++) {
			...
			if (i == DISK_INDEX) {
				try {
					// try to check whether a disk amount was passed, stays at
					// default value, if none was passed
			...
			if (value.contains("delay")) {
				...
				// It is expected that there is a delay provided if the
				// arguments contain "delay"
